...
/* ── PHOTO : gère 3 cas → URL/chemin (png/jpg), base64 brut, ou échec ── */
function setPhoto(e){
  var ph=document.getElementById('profile-photo');
  if(!ph) return;

  var src=e.photo_b64||e.photo||e.photo_url||'';
  src=String(src).trim();

  /* Placeholder avec initiales si la photo échoue */
  ...
  /* PDF non supporté par <img> */
  if(/\.pdf$/i.test(src)){
    console.warn('Deciplan: la photo est au format PDF. Convertissez-la en .png ou .jpg.');
    showInitials();
    return;
  }

  /* Détection : URL, chemin relatif, ou nom de fichier image */
  var isPath = /^https?:\/\//i.test(src)
            || src.indexOf('/')>=0
            || /\.(jpe?g|png|webp|gif|svg)$/i.test(src);
  ...
  if(isPath){
    ...
    /* Base64 brut : on détecte le type d'image par le préfixe */
    var mime='image/jpeg';
...
function renderProfil(e){
  ...
  if(nomEl){
    var nomComplet='';
    if(!isEmpty(e.prenom)) nomComplet+=e.prenom;
    if(!isEmpty(e.nom)){
      if(nomComplet) nomComplet+=' ';
      nomComplet+=e.nom;
    }
    nomEl.textContent=nomComplet||'Étudiant(e)';
  }
  ...
  /* Remarque : accepte remarque OU remarque_conseiller */
...
document.addEventListener('DOMContentLoaded',function(){
  ...
  fetch('etudiants.json')
    ...
    .catch(function(err){
      console.error('Erreur de chargement etudiants.json :',err);
      var nb=document.getElementById('nb-cand');
      if(nb) nb.textContent='Erreur de chargement';
    });
});
